...
知识库是关键。Jev 只能在已经编号的控件里选题，能输入的字也只能来自目标里已经写出的短语。它不知道百度结果里哪条是广告、左边那个城市框才是出发地、登录弹窗出现就该停。这些写在 `knowledge/<应用>/*.md`。

现场跑失败，多半是笔记缺了或写错了，不是缺一个选择器。先看报告，写清模型该点哪个、不该点哪个，再补进对应笔记。`hosts` 对站点，`keywords` 对目标。`general: true` 每次运行都带上，例如遇到登录弹窗就停止。笔记只是参考，模型仍然只能选当前页上观测到的索引。不要把某个站点的填写规则写回 `src/policy.ts`。
